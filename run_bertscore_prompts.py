...
def main():
    ...
    if args.data=='dolly':
        dolly = load_dataset("databricks/databricks-dolly-15k")["train"].to_pandas().instruction.sample(args.samples, random_state=1).values.tolist()
        ...
        data = load_dataset("HuggingFaceH4/no_robots")["train"].to_pandas().prompt.sample(args.samples, random_state=1).values.tolist()
    elif args.data=='cnn':        
